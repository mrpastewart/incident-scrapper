...
$table = "lewes";
$url = "http://www.lewesfire.com/incidents";
$email_url = $url;
$agency_name = "Lewes, Delaware";
...
foreach ($lines as $line) {
	...
	if (ereg("class=.date.>", $line)) {
		$ctr = 1;
		$date_time = ereg_replace(".*date.>", "", $line);
		$date_time = ereg_replace("</div>.*", "", $date_time);
		$date_time = ereg_replace("</em>.*", "", $date_time);
		continue;
	}
	...
	if (ereg("<dd>", $line) && $ctr == 2) {
		...
		$description = ereg_replace(".*<dd>", "", $description);
		$description = ereg_replace("</dd>.*", "", $description);
		$address = "";
		continue;
	}

	if (ereg("Cross Streets|</div>", $line) && $ctr == 3) {
		$ctr = 5;
	}

	if (ereg("<address>", $line) && $ctr == 3) {
		$ctr = 4;
	}

	if (ereg("City:|Address:|Location:", $line) && $ctr == 3) {
		$ctr++;
		continue;
	}
