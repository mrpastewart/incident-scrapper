...
$table = "houston";
//$url = "http://cbtcws.cityofhouston.gov/activeincidents/hfdincidents.aspx";
$url = "http://cohweb.houstontx.gov/ActiveIncidents/HFDIncidents.aspx";
$email_url = $url;
$agency_name = "Houston, Texas";
...
$page = ereg_replace(".*>Alarm Level<", "", $page);
...
foreach ($lines as $line) {
	...
	list($f1, $f2, $f3, $f4, $f5, $f6) = split("</td>", $line);

	$address = trim(ereg_replace(".*\">", "", $f1));
	$timestamp = trim(ereg_replace(".*\">", "", $f4));
	$description = trim(ereg_replace(".*\">", "", $f5));
	$alarm_level = trim(ereg_replace(".*\">", "", $f6));

	$description .= " (level $alarm_level)";

	if (eregi("ems event", $description)) {
		continue;
	}
