...
foreach ($lines as $line) {
	if (ereg("<div class=.date.>", $line)) {
		$ctr = 1;

		$line = ereg_replace(".*<div class=.date.>", "", $line);
...
//echo "description> $description\n";
	...
	if (ereg("<dt>Address:</dt>|<dt>Location:</dt>|<dt>City:</dt>", $line) && $ctr > 2) {
		if (ereg("<dd>", $line)) {
			$line = ereg_replace(".*<dd>", "", $line);
			$line = ereg_replace("</dd>.*", "", $line);
...
//echo "address> $address\n";
	...
	if (! ereg("<dd>", $line) && ereg("</dd>", $line) && $ctr == 4) {
		$ctr++;
...
*/
	if (ereg("<div class=.sep2.", $line) && $ctr > 3 && $ctr < 9) {
		$ctr = 9;
	}
