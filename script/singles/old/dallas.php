...
$page = ereg_replace(".*INCIDENTS TABLE STARTS HERE", "", $page);
...
foreach ($lines as $line) {
	...
	if (ereg("nowrap.>", $line) && $ctr == 1) {
		$ctr++;
		$line = ereg_replace(".*nowrap.>", "", $line);
		$description = ereg_replace("</td>.*", "", $line);
		continue;
	}

	if (ereg("nowrap.>", $line) && $ctr == 2) {
		$ctr++;
		$line = ereg_replace(".*target=._blank.>", "", $line);
		$address = ereg_replace("</a>.*", "", $line);
		continue;
	}
...
$result = mysql_query("select time_format(date_add(time, interval $offset second), '%H:%i:%s') as timex, address, description from $table where sent = 0 order by time desc");
