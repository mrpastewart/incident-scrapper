...
$email_url = $url;
$agency = "Pike County, Pennsylvania";
...
$page = ereg_replace(".*>Units Assigned</td>", "", $page);
...
foreach ($lines as $line) {
	...
	if (ereg("<td class=.table-data", $line) && $ctr == 1) {
		$ctr = 2;
		$line = ereg_replace(".*<br>", "", $line);
		$date_time = ereg_replace("&nb.*", "", $line);
		continue;
	}

	if (ereg("<td class=.table-data", $line) && $ctr == 2) {
		$ctr = 3;
		$line = ereg_replace(".*table-data.>", "", $line);
		$description = ereg_replace("&nb.*", "", $line);
		continue;
	}

	if (ereg("<td id=.rptResults_ctl00_tdLocation", $line) && $ctr == 3) {
		$ctr = 4;
		$line = ereg_replace(".*table-data.>", "", $line);
...
$result = mysql_query("select time_format(time, '%H:%i:%s') as timex, description, address, updated from $table where sent = 0 order by time desc");
