...
$table = "roxana";
$url = "http://www.roxana90.com/runlog.cfm";
$email_url = $url;
$agency_name = "Roxana VFD, Frankford, Delaware";
...
$page = ereg_replace("Designed By.*", "", $page);

$lines = explode("\n", $page);

foreach ($lines as $line) {
	if (ereg("<div class=.tag9.>", $line)) {
		$ctr = 1;
	...
	if (ereg(">Nature:<", $line) && $ctr == 1) {
		...
		if (ereg(">Location:<", $line)) {
			...
			$location = ereg_replace(".*/span> *", "", $location);
		...
		$description = ereg_replace(".*</span> *", "", $line);
