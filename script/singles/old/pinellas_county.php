...
$table = "pinellas_county";
$url = "http://www.pinellascounty.org/911/activity.xml";
$email_url = "http://www.pinellascounty.org/911/ActCallsPub.htm";
$agency_name = "Pinellas County, Florida";
...
$page = ereg_replace("</Location>", "</Location>\n", $page);
$page = ereg_replace("</Type>", "</Type>\n", $page);
...
foreach ($lines as $line) {
	if (ereg("<Location>", $line)) {
		...
		$address = ereg_replace("</Location>.*", "", $line);
		continue;
	}

	if (ereg("<Type>", $line) && ! ereg("><Type>", $line)) {
		$line = ereg_replace(".*<Type>", "", $line);
		$description = ereg_replace("</Type>.*", "", $line);
	}
