...
$table = "bridgeville";
$url = "http://bridgeville72.com/incidents";
$email_url = $url;
$agency_name = "Bridgeville, Delaware";
...
$page = ereg_replace(".*<span>Run Log</span>", "", $page);
...
foreach ($lines as $line) {
	...
	if (ereg("<dd>", $line) && $ctr == 2) {
		...
		$line = ereg_replace(".*<dd>", "", $line);
		$description = ereg_replace(".*Nature: *", "", $line);

		continue;
	}

	if (ereg(">Location:|>City:|>Address:", $line) && $ctr == 3) {
		$line = ereg_replace("</dd>.*", "", $line);
		$line = ereg_replace(".*<dd>", "", $line);
