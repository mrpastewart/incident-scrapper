...
$table = "mid_sussex";
$url = "http://www.midsussexrescuesquad.com/runlog.cfm";
$email_url = $url;
$agency_name = "Mid-Sussex Rescue Squad, Millsboro, Delaware";
...
foreach ($lines as $line) {
	if (ereg("headline3", $line)) {
		$ctr = 1;
		continue;
	}

	if (ereg("<center>", $line) && $ctr == 1) {
		$ctr++;
		continue;
	}
	...
	if (ereg("City:|Location:", $line) && $ctr >= 6) {
		$ctr = 7;
		continue;
	}

	if (ereg("</td>", $line) && $ctr == 7) {
		$ctr = 6;
		continue;
	}

	if (ereg("<td class=", $line) && $ctr == 7) {
		continue;
	}
