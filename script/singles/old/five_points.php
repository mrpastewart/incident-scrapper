...
$table = "five_points";
$url = "http://www.fivepoints17.com/Incidents";
$email_url = $url;
$agency_name = "Five Points VFD, Delaware";
...
$page = ereg_replace(".*Contact Us", "", $page);
$page = ereg_replace("<div class=.paging.*", "", $page);

$lines = explode("\n", $page);

foreach ($lines as $line) {
	if (ereg("home_run", $line)) {
		$ctr = 1;
		continue;
	}

	if (ereg("<div class=.date.", $line) && $ctr == 1) {
		$ctr++;

		$line = ereg_replace(".*left;.>", "", $line);
	...
	if (ereg("Nature:", $line) && $ctr == 2) {
		...
		if (ereg("<strong>Location:</strong>", $line)) {
			$address = ereg_replace(".*<dd> *", "", $line);
			$address = ereg_replace("</dd>.*", "", $address);
	...
	if (ereg("<strong>Location:</strong>|<strong>City:</strong>|<strong>Address:</strong>", $line) && $ctr == 3) {
		if (strlen($address) > 0) {
			$address .= ", ";
		}
