...
$email_url = $url;
$agency_name = "Mill Creek, Delaware";
...
foreach ($lines as $line) {
	...
	if (ereg("<dt>Nature:</dt>", $line) && $ctr == 1) {
		...
		if (ereg("<dt>Location:</dt>", $line)) {
			$address = ereg_replace(".*<dt>Location.*<dd>", "", $line);
			$address = ereg_replace("</dd>.*", "", $address);
			$address = ereg_replace("<br/>.*", "", $address);

			$line = ereg_replace("<dt>Location:.*", "", $line);
		}
	...
	if (ereg("<dt>Address:</dt>|<dt>City:</dt>", $line) && $ctr == 2) {
		$ctr++;

		if (! (ereg("<dd>", $line) && ! ereg("</dd>", $line))) {
			$ctr++;
		}
