...
$table = "washington_county_md";
$url = "https://access.active911.com/interface/js.php?r3vd581";
$email_url = "http://alertpage.net/washcad/";
$agency_name = "Washington County, Maryland";
...
$page = ereg_replace(".*a91.data=", "", $page);
$page = ereg_replace(".*.alarms.:", "", $page);
...
foreach ($lines as $line) {
	...
	$description = ereg_replace(".,.generic_title.*", "", $line);
	...
	$address = ereg_replace(".,.city.*", "", $line);
	$address = ereg_replace(".*address.:.", "", $address);

	$city = ereg_replace(".,.state.*", "", $line);
	$city = ereg_replace(".*city.:.", "", $city);
	...
	// seen?
	$result = mysql_query("select time from $table where description = '" . mysql_real_escape_string($description) . "' and address = '" . mysql_real_escape_string($address) . "' and time > date_sub(now(), interval 1 day)");
	...
	$sql = "insert into $table (time, address, description) values (now(), '" . mysql_real_escape_string($address) . "', '" . mysql_real_escape_string($description) . "')";
