...
$email_url = $url;
$agency_name = "Delaware City, Delaware";
...
$page = ereg_replace(".*liverunlog.gif", "", $page);
$page = ereg_replace("pagingTabsCarr.*", "", $page);

$lines = explode("\n", $page);

foreach ($lines as $line) {

	if (ereg("class=.sch3c.>", $line)) {
		$ctr = 1;
		$date_time = ereg_replace(".*sch3c.>", "", $line);
		$date_time = ereg_replace("</div>.*", "", $date_time);
		continue;
	}
	...
	if (ereg("City:|Address:|Location:", $line) && $ctr == 4) {
		$ctr++;
		continue;
	}

	if (ereg("class=.gbr.", $line) && $ctr == 5) {
		$ctr++;
		continue;
	}
	...
	if (ereg("/table", $line) && $ctr > 2 && $ctr < 7) {
		$ctr = 7;
	}
